...
export async function getCurrentUserScore(req, res) {
    try {
        const userId = req.user.userId;
        const user = await prisma.user.findUnique({
            where: { id: userId },
...
export async function submitTypingGameScore(req, res) {
    try {
        const userId = req.user.userId;
        const { wpm, accuracy, wordsTyped, charactersCorrect, timeElapsed } = req.body;
        ...
        if (typeof wpm !== "number" ||
            ...
            res.status(400).json({
                error: "Invalid game data. All fields (wpm, accuracy, wordsTyped, charactersCorrect, timeElapsed) are required.",
            });
            return;
        }
        ...
        if (timeElapsed < 50 || timeElapsed > 70) {
            res.status(400).json({
                error: "Invalid game duration. Game must be completed in approximately 60 seconds.",
            });
            return;
        }
        ...
        if (recentSubmission) {
            res.status(429).json({
                error: "Please wait before submitting again. Duplicate submissions are not allowed.",
            });
            return;
        }
        // Get current user
...
export async function getTypingGameHistory(req, res) {
    try {
        const userId = req.user.userId;
        ...
        const history = actionLogs
            ...
            const match = log.action.match(/TYPING_GAME:(\d+(?:\.\d+)?)WPM:(\d+(?:\.\d+)?)%:(\d+)words/);
